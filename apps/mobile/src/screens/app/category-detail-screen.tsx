...
} from "@/components/category-color-picker";
...
export function CategoryDetailScreen() {
  ...
  const route =
    useRoute<RouteProp<CategoriesStackParamList, "CategoryDetail">>();
  ...
  const [contactPageInfo, setContactPageInfo] =
    useState<ContactPageInfo | null>(null);
  const contactRequestVersion = useRef(0);
  const initializedContactCategoryId = useRef<string | null>(null);

  const category =
    selectedCategory ?? categories.find((item) => item.id === categoryId);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      void getMobileSubscription()
        .then((response) =>
          setContactAccess(
            response.subscription?.entitlements.contactMessaging ?? false,
          ),
        )
  ...
  useEffect(() => {
    ...
    const assignedIds = groups
      .filter((group) =>
        group.categories.some((item) => item.category.id === categoryId),
      )
      .map((group) => group.id);
  ...
  const loadContacts = useCallback(
    async (page = 1, append = false) => {
      ...
      try {
        const response = await getMobileCategoryContacts(categoryId, {
          page,
          limit: 50,
  ...
  const assignedGroups = useMemo(
    () => groups.filter((group) => assignmentGroupIds.includes(group.id)),
    [assignmentGroupIds, groups],
  );
  ...
  const saveCategory = async () => {
    ...
    const ok = await updateCategory(category.id, {
      ...
      ...(contactAccess
        ? { contactIds: [...new Set(selectedContactIds)] }
        : {}),
    });
    if (ok)
      await Promise.all([
        loadCategories(),
        loadGroups(),
        contactAccess ? loadContacts(1, false) : Promise.resolve(),
      ]);
  };
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
              ...
              <View style={styles.detailTitleRow}>
                <View
                  style={[
                    styles.colorDot,
                    {
                      backgroundColor: color || category.color || theme.primary,
                    },
                  ]}
                />
            ...
            {validationError ? (
              <Feedback text={validationError} tone="error" />
            ) : null}
              ...
              <View style={styles.assignedList}>
                {assignedGroups.map((group) => (
                  <AssignableGroup
                    ...
                    selected
                    compact
                    onToggle={() => toggleAssignment(group.id)}
                  />
                ))}
              </View>
            ) : (
              <EmptyState
                title={t("noAssignedGroups")}
                description={t("noAssignedGroupsDescription")}
              />
            )}
            ...
            {!groups.length ? (
              <EmptyState
                title={t("noAssignableGroups")}
                description={t("noAssignableGroupsDescription")}
              />
            ) : null}
          </View>
        }
        ListEmptyComponent={
          groups.length ? (
            <EmptyState
              title={t("allGroupsAssigned")}
              description={t("allGroupsAssignedDescription")}
            />
          ) : null
        }
        renderItem={({ item }) => (
          ...
          <View style={styles.footer}>
            <View style={styles.sectionHeaderRow}>
              <View>
                ...
                <Text style={[styles.meta, { color: theme.muted }]}>
                  {t("contactsSelected", { count: selectedContactIds.length })}
                </Text>
              </View>
              {contactAccess ? (
                <Pressable
            ...
            {!contactAccess ? (
              ...
              >
                ...
                <Text style={[styles.lockedText, { color: theme.text }]}>
                  {t("contactCategoryProfessionalRequired")}
                </Text>
              </View>
            ) : (
              <>
                ...
                >
                  ...
                  >
                    <Text
                      style={[styles.checkmark, { color: theme.primaryText }]}
                    >
                      {contacts.length > 0 &&
                      contacts.every((contact) =>
                        selectedContactIds.includes(contact.id),
                ...
                {loadedAssignedContacts.length ? (
                  <View style={styles.contactList}>
                    {loadedAssignedContacts.map((contact) => (
                      <AssignableContact
                        key={`assigned-${contact.id}`}
                        contact={contact}
                        selected
                  ...
                  <Text style={[styles.meta, { color: theme.muted }]}>
                    {t("noAssignedContactsInView")}
                  </Text>
                )}

                <Text style={[styles.subsectionTitle, { color: theme.muted }]}>
                  {t("assignableContacts")}
                </Text>
                {contactError ? (
                  <Feedback text={contactError} tone="error" />
                ) : null}
                {!contacts.length && contactLoading ? (
                  <LoadingState label={t("contactsLoading")} />
                  ...
                  <EmptyState
                    title={
                      contacts.length
                        ? t("allContactsAssigned")
                        : t("contactNotFound")
                    }
                    description={
                      contacts.length
                        ? t("allVisibleContactsAssigned")
                        : t("contactSearchHelp")
                    }
                  />
                )}
                {contactPageInfo?.hasMore ? (
                  <PrimaryButton
                    ...
                    disabled={contactLoading}
                    onPress={() =>
                      void loadContacts(contactPageInfo.page + 1, true)
                    }
                  />
                ) : null}
              </>
            ...
            >
              <Text style={[styles.saveCount, { color: theme.text }]}>
                {audienceSummary}
              </Text>
              <PrimaryButton
                title={t("saveChanges")}
  ...
  return (
    ...
    >
      ...
      <View style={styles.groupBody}>
        ...
        <Text style={[styles.meta, { color: theme.muted }]}>
          {compact
            ? t("tapToRemoveCategory")
            : `${formatNumber(group.participantCount ?? 0, locale)} ${t("members")} · ${group.canSend ? t("sendable") : t("notSendable")}`}
        </Text>
      </View>
    </Pressable>
  );
  ...
  return (
    ...
    >
      ...
      <View style={styles.groupBody}>
        ...
        {getMobileContactPhoneLabel(contact) ? (
          <Text style={[styles.meta, { color: theme.muted }]}>
            {getMobileContactPhoneLabel(contact)}
          </Text>
        ) : null}
...
function Feedback({ text, tone }: { text: string; tone: "success" | "error" }) {
  ...
  const backgroundColor =
    tone === "success" ? theme.successSoft : theme.dangerSoft;

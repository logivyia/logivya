...
import { confirmMfaEnrollment, disableMfa, getMfaStatus, regenerateMfaRecoveryCodes, revokeMfaTrustedDevice, startMfaEnrollment, type MfaEnrollment, type MfaStatus } from "@/api/mfa-api";
...
export function SecurityScreen() {
  ...
  async function begin() {
    ...
    try {
      ...
      setEnrollment(result); setRecoveryCodes(result.recoveryCodes); setCode("");
    } catch (error) { Alert.alert(t("security"), error instanceof Error ? error.message : t("operationFailedError")); }
  ...
  async function confirm() {
    setBusy(true);
    try { await confirmMfaEnrollment(code); setEnrollment(null); setCode(""); await load(); Alert.alert(t("security"), t("mfaEnabledSuccess")); }
    catch (error) { Alert.alert(t("security"), error instanceof Error ? error.message : t("operationFailedError")); }
    finally { setBusy(false); }
  }
  ...
  async function regenerate() {
    setBusy(true);
    try { const result = await regenerateMfaRecoveryCodes(code); setRecoveryCodes(result.recoveryCodes); setCode(""); await load(); }
    catch (error) { Alert.alert(t("security"), error instanceof Error ? error.message : t("operationFailedError")); }
    finally { setBusy(false); }
  }
  ...
  return <Screen style={styles.screen}><ScrollView contentContainerStyle={styles.content}>
    ...
    <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
      ...
      {!status?.enabled && !enrollment ? <PrimaryButton title={t("mfaEnable")} icon="key-outline" loading={busy} onPress={begin} /> : null}
      {enrollment ? <View style={styles.stack}><View style={styles.qrPanel}><Image source={{ uri: enrollment.qrCodeDataUrl }} style={styles.qr} /></View><Text style={[styles.label, { color: theme.muted }]}>{t("mfaManualKey")}</Text><Text selectable style={[styles.mono, { color: theme.text, backgroundColor: theme.cardMuted }]}>{enrollment.secret}</Text><TextField label={t("mfaCode")} value={code} autoCapitalize="characters" onChangeText={setCode} /><PrimaryButton title={t("mfaConfirmEnable")} loading={busy} disabled={code.trim().length < 6} onPress={confirm} /></View> : null}
      {status?.enabled ? <View style={styles.stack}><TextField label={t("password")} secureTextEntry value={password} onChangeText={setPassword} /><TextField label={t("mfaCode")} value={code} autoCapitalize="characters" onChangeText={setCode} /><Pressable disabled={busy || !password || code.trim().length < 6} onPress={disable} style={[styles.dangerButton, { borderColor: theme.danger }]}><Text style={[styles.dangerText, { color: theme.danger }]}>{t("mfaDisable")}</Text></Pressable></View> : null}
    </View>
    {recoveryCodes.length || status?.enabled ? <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}><Text style={[styles.cardTitle, { color: theme.text }]}>{t("mfaRecoveryCodes")}</Text><Text style={[styles.body, { color: theme.muted }]}>{t("mfaRecoveryRemaining", { count: status?.recoveryCodesRemaining ?? recoveryCodes.length })}</Text>{recoveryCodes.length ? <><Text selectable style={[styles.recovery, { color: theme.text, backgroundColor: theme.cardMuted }]}>{recoveryCodes.join("\n")}</Text><Pressable onPress={() => void Clipboard.setStringAsync(recoveryCodes.join("\n"))} style={styles.textButton}><Ionicons name="copy-outline" size={18} color={theme.primary} /><Text style={{ color: theme.primary, fontWeight: "800" }}>{t("mfaCopyCodes")}</Text></Pressable></> : null}{status?.enabled ? <><TextField label={t("mfaCode")} value={code} autoCapitalize="characters" onChangeText={setCode} /><PrimaryButton title={t("mfaRegenerate")} icon="refresh" loading={busy} disabled={code.trim().length < 6} onPress={regenerate} /></> : null}</View> : null}

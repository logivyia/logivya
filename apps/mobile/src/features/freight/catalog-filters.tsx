...
export function CatalogFilters({ value, onApply }: { value: MarketplaceFilters; onApply: (value: MarketplaceFilters) => void }) {
  ...
  const label = (key: string) => ({ originCountry: locale === "uz" ? uz.originCountry : tr ? "Kalkış ülkesi" : "Origin country", destinationCountry: locale === "uz" ? uz.destinationCountry : tr ? "Varış ülkesi" : "Destination country", originCity: locale === "uz" ? uz.originCity : tr ? "Kalkış şehri / ilçesi" : "Origin city / district", destinationCity: locale === "uz" ? uz.destinationCity : tr ? "Varış şehri / ilçesi" : "Destination city / district", vehicle: locale === "uz" ? uz.vehicle : tr ? "Araç tipi" : "Vehicle type" })[key];
  const button = [styles.button, { borderColor: theme.border, backgroundColor: theme.card }];
  const titleFor = (key: "originCountry" | "destinationCountry" | "vehicle", current: string) => (locale === "uz" ? (key === "vehicle" ? uzbekVehicleLabels : uzbekCountryLabels)[current] : undefined) ?? (key === "vehicle" ? marketplaceVehicles : marketplaceCountries).find(([code]) => code === current)?.[1] ?? (locale === "uz" ? uz.all : tr ? "Tümü" : "All");
  return <View style={{ marginVertical: 12 }}>
    ...
    <Modal visible={open} animationType="slide" onRequestClose={() => select ? setSelect(null) : setOpen(false)}>
      <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}><View style={styles.heading}><Text style={{ color: theme.text, fontSize: 22, fontWeight: "900", flex: 1 }}>{select ? label(select) : locale === "uz" ? uz.title : tr ? "İlan filtreleri" : "Listing filters"}</Text><Pressable style={button} accessibilityLabel={locale === "uz" ? uz.close : tr ? "Kapat" : "Close"} onPress={() => select ? setSelect(null) : setOpen(false)}><Ionicons name={select ? "arrow-back" : "close"} size={23} color={theme.text} /></Pressable></View>
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 20, gap: 16 }}>
        {select ? [["", locale === "uz" ? uz.all : tr ? "Tümü" : "All"], ...(select === "vehicle" ? marketplaceVehicles : marketplaceCountries)].map(([code, title]) => <Pressable key={code} style={button} onPress={() => { setDraft({ ...draft, [select]: code }); setSelect(null); }}><Text style={{ color: theme.text, flex: 1 }}>{locale === "uz" ? (select === "vehicle" ? uzbekVehicleLabels : uzbekCountryLabels)[code] ?? title : title}</Text>{draft[select] === code ? <Ionicons name="checkmark" size={22} color={theme.primary} /> : null}</Pressable>) : <>
          ...
          <Pressable style={[button, { backgroundColor: theme.primary }]} onPress={() => { onApply(draft); setOpen(false); }}><Text style={{ color: theme.primaryText, fontWeight: "900" }}>{locale === "uz" ? uz.applyFilters : tr ? "Filtreleri uygula" : "Apply filters"}</Text></Pressable><Pressable style={button} onPress={() => { const cleared = { ...emptyMarketplaceFilters, kind: value.kind }; setDraft(cleared); onApply(cleared); setOpen(false); }}><Text style={{ color: theme.text }}>{locale === "uz" ? uz.clearFilters : tr ? "Filtreleri temizle" : "Clear filters"}</Text></Pressable>
        </>}

...
| Android permissions | Startup regression identified and corrected | Broad storage/photo and advertising identifiers remain removed; backup and cleartext remain disabled. `DETECT_SCREEN_CAPTURE` must remain because `expo-screen-capture` registers the Android 14+ callback during native startup. Removing it caused the v130-v132 startup crash before login. | Require the permission in source preflight and AAB verification, then run an Android 14+ cold-start and in-place-update smoke test for every release. |

...
Logivya must behave as one SaaS product across desktop web, mobile web, Android, and iOS.
...
- Desktop web, mobile web, Android, and iOS must use the same backend contract.
...
- Business logic must not be forked per client. Web, mobile web, Android, and iOS are presentation layers over backend-owned workflows.
- Every new feature must be designed, reviewed, and tested against desktop web, mobile web, Android, and iOS before release.
...
Android and iOS share the Expo/React Native client. Platform-specific native behavior must still be verified on a real device before production release.
...
- iOS workflow exists in the shared native client and is covered by an explicit TestFlight/device test when native behavior differs.
...
The evidence matrix for the dashboard, subscription, authentication, and security release is maintained in `docs/web-mobile-parity-v138.md`.

...
- Examples: Android app startup time, network latency/status, screen rendering performance, app version and device class.
- Purpose: reliability, performance improvement and technical troubleshooting.
- Shared with third parties: Firebase Performance Monitoring only after the user enables crash/diagnostic data sharing.
- Default behavior: disabled until the optional diagnostics preference is granted; the user can withdraw it in Privacy settings.
- Excluded: message bodies, passwords, authentication tokens, contact names and WhatsApp content must not be added as custom attributes.
- Encrypted in transit: yes.

...
assertIncludes(provider, "resolveWhatsAppLinkedDeviceMetadata", "phone pairing must centralize browser identity resolution");
assertIncludes(linkedDeviceMetadata, "Browsers.ubuntu", "phone pairing must use Baileys' supported Ubuntu browser tuple by default");
assertIncludes(linkedDeviceMetadata, "WHATSAPP_LINKED_DEVICE_NAME", "phone pairing must support a canonical linked-device client name");
assertIncludes(linkedDeviceMetadata, "WHATSAPP_PAIRING_BROWSER_NAME", "phone pairing must preserve the legacy browser-name environment alias");

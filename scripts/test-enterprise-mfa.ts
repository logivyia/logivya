...
async function main() {
  ...
  const webLoginForm = readFileSync(resolve("src/components/auth-form.tsx"), "utf8");
  const mobileLoginScreen = readFileSync(resolve("apps/mobile/src/screens/auth/login-screen.tsx"), "utf8");
  const mobileDrawer = readFileSync(resolve("apps/mobile/src/components/web-parity-tab-bar.tsx"), "utf8");
  const mobileSecurityScreen = readFileSync(resolve("apps/mobile/src/screens/app/security-screen.tsx"), "utf8");
  ...
  assert(webSession.includes("resolveMfaLoginDecision") && webSession.includes("!mfa.policySatisfied"), "Web API sessions must enforce completed MFA and organization policy");
  assert(mobileSession.includes("resolveMfaLoginDecision") && mobileSession.includes("!mfa.policySatisfied"), "Mobile API sessions must enforce completed MFA and organization policy");
  assert(mobileSession.includes("mobileSessionSatisfiesMfaPolicy") && mobileSession.includes("if (!mfaSatisfied)"), "Mobile refresh must not bypass MFA or organization policy");
  ...
  assert(webLoginForm.includes("normalizeMfaLoginCode") && mobileLoginScreen.includes("normalizeMfaLoginCode"), "Web and mobile login must accept normalized recovery codes");
  assert(webLoginForm.includes("? 6 : 64") && mobileLoginScreen.includes("? 6 : 64"), "Recovery codes must not be truncated to six digits");
  assert(mobileDrawer.includes('screen: "Security"'), "The native Security screen must be reachable from the mobile drawer");
  assert(mobileSecurityScreen.includes("usePreventScreenCapture"), "The native Security screen must protect sensitive enrollment data");

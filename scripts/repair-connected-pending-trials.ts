...
function readLimit() {
  ...
  return Number.isFinite(parsed) ? Math.min(Math.max(Math.trunc(parsed), 1), 500) : 100;
}
...
async function main() {
  ...
  const { reconcileConnectedPendingTrials } = await import("../src/server/billing/trial-service");
  ...
  try {
    ...
    console.log(JSON.stringify({ mode: apply ? "apply" : "audit", count: candidates.length, candidates }, null, 2));
    if (!apply) {
      console.log("Audit only. Re-run with --apply after reviewing the candidate list.");
      return;
    }

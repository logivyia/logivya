...
const expectedExistingPhoneImageIds = [
  "7564822785106812595",
  "5041096390734529830",
  "10418868337790978980",
  "15550232916261934164",
  "5483172430938681801",
];
const preservedImageTypes = ["sevenInchScreenshots", "tenInchScreenshots"];
// Google Play allows at most eight phone screenshots. Keep the five live
// product screenshots and append the three strongest non-duplicative views.
const sourceFiles = [6, 8, 9].map((index) =>
  path.join(repoRoot, `screenap${index}.jpeg`));
...
async function listImages(token, editId, type) {
  const payload = await request(token, editPath(editId, `/listings/${language}/${type}`));
...
async function verifyPreservedImages(token, editId, baseline) {
  for (const type of preservedImageTypes) {
    ...
    if (!sameIds(currentIds, baseline[type])) throw new Error(`${type} changed unexpectedly.`);
  }
}
...
async function verifyCommittedState(token, expectedPhoneIds, preservedBaseline) {
  const verificationEdit = await createEdit(token);
  try {
    const finalPhoneIds = ids(await listImages(token, verificationEdit.id, imageType));
    if (!sameIds(finalPhoneIds, expectedPhoneIds) || finalPhoneIds.length !== 8) {
...
async function main() {
  ...
  let committed = false;
  try {
    const existingPhoneIds = ids(await listImages(token, edit.id, imageType));
    if (!sameIds(existingPhoneIds, expectedExistingPhoneImageIds)) {
      throw new Error(`Phone screenshots changed since verification: ${existingPhoneIds.join(", ")}`);
    }
    ...
    const expectedPhoneIds = [...existingPhoneIds, ...uploadedIds];
    const stagedPhoneIds = ids(await listImages(token, edit.id, imageType));
    if (!sameIds(stagedPhoneIds, expectedPhoneIds) || stagedPhoneIds.length !== 8) {
    ...
    committed = true;
    const verified = await verifyCommittedState(token, expectedPhoneIds, preservedBaseline);
    console.log(JSON.stringify({
      ok: true,
      packageName,
      language,
      preservedPhoneImageIds: existingPhoneIds,
      appendedPhoneImageIds: uploadedIds,
      finalPhoneImageIds: verified.finalPhoneIds,

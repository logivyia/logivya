...
release="favicon-circle-v5-20260902-7"
base_web_image="logivya-web:legal-heading-contrast-20260902-6"
base_worker_image="logivya-worker:live-market-arabic-20260902-4"
new_web_image="logivya-web:${release}"
patch_archive=${1:?usage: deploy-favicon-circle-v5-20260902.sh <patch-archive> <patch-sha256> <favicon-sha256>}
expected_patch_sha=${2:?patch sha256 is required}
expected_favicon_sha=${3:?favicon sha256 is required}
base_source_dir="/opt/logivya/releases/rc-source-legal-heading-contrast-20260902-6"
...
fail_report() {
  rc=$?
  trap - ERR
  printf '{"ok":false,"audit":"FAVICON_CIRCLE_V5_DEPLOY_STOP","stage":"%s","exitCode":%s}\n' "${stage}" "${rc}" >&2
  exit "${rc}"
}
...
test -f "${source_dir}/public/logivya/favicon-circle-white-master.jpg"
test "$(sha256sum "${source_dir}/public/favicon-512x512.png" | cut -d' ' -f1)" = "${expected_favicon_sha}"
grep -q '/favicon.ico?v=5' "${source_dir}/src/app/layout.tsx"
grep -q '/android-chrome-512x512.png?v=5' "${source_dir}/src/app/manifest.ts"
...
sudo docker exec "${web_canary}" /usr/local/bin/logivya-entrypoint node -e '
  ...
  Promise.all([fetch("http://127.0.0.1:3000/"), fetch("http://127.0.0.1:3000/favicon-512x512.png?v=5")]).then(async ([page, icon]) => {
    const html = await page.text();
    const hash = crypto.createHash("sha256").update(Buffer.from(await icon.arrayBuffer())).digest("hex");
    if (page.status !== 200 || icon.status !== 200 || !html.includes("/favicon.ico?v=5") || hash !== process.argv[1]) process.exit(70);
...
rollback() {
  ...
  printf '{"ok":false,"audit":"FAVICON_CIRCLE_V5_DEPLOY_ROLLED_BACK","stage":"%s","exitCode":%s}\n' "${stage}" "${rc}" >&2
  exit "${rc}"
}
trap 'rollback $?' ERR
...
test "$(curl --connect-timeout 5 --max-time 20 -sS https://www.logivya.com/favicon-512x512.png?v=5 | sha256sum | cut -d' ' -f1)" = "${expected_favicon_sha}"
curl --connect-timeout 5 --max-time 20 -fsS https://www.logivya.com/ | grep -q '/favicon.ico?v=5'
...
trap - ERR INT TERM HUP
activation_started=false
printf '{"ok":true,"audit":"FAVICON_CIRCLE_V5_PRODUCTION_DEPLOYED","release":"%s","webImage":"%s","sourceHash":"%s","patchSha256":"%s","faviconSha256":"%s","workerContainersUnchanged":true,"webCanary":true}\n' \
  "${release}" "${new_web_image}" "${source_hash}" "${expected_patch_sha}" "${expected_favicon_sha}"

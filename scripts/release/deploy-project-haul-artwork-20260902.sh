...
release="project-haul-artwork-20260902-10"
base_web_image="logivya-web:trial-copy-dedupe-20260902-9"
base_worker_image="logivya-worker:live-market-arabic-20260902-4"
new_web_image="logivya-web:${release}"
artwork_sha="9490ea70b977d4d482c722d33ab8f9ee97df7ad6157cf63971d67afef598dd24"
patch_archive=${1:?usage: deploy-project-haul-artwork-20260902.sh <patch-archive> <patch-sha256>}
expected_patch_sha=${2:?patch sha256 is required}
base_source_dir="/opt/logivya/releases/rc-source-trial-copy-dedupe-20260902-9"
...
fail_report() {
  rc=$?
  trap - ERR
  printf '{"ok":false,"audit":"PROJECT_HAUL_ARTWORK_DEPLOY_STOP","stage":"%s","exitCode":%s}\n' "${stage}" "${rc}" >&2
  exit "${rc}"
}
...
test "$(sha256sum "${source_dir}/public/logivya/project-haul-icon-transparent.png" | cut -d' ' -f1)" = "${artwork_sha}"
grep -q 'project-haul-icon-transparent.png' "${source_dir}/src/components/lowbed-icon.tsx"
grep -q 'LowbedIcon' "${source_dir}/src/components/app-shell.tsx"
...
sudo env BUILDKIT_PROGRESS=plain docker build --progress plain \
  ...
  --build-arg "NEXT_PUBLIC_APP_VERSION=1.0.193" \
  ...
  --label "org.opencontainers.image.artwork-sha256=${artwork_sha}" \
...
sudo docker exec "${web_canary}" /usr/local/bin/logivya-entrypoint node -e '
  ...
  Promise.all([
    ...
    fetch("http://127.0.0.1:3000/logivya/project-haul-icon-transparent.png"),
  ]).then(async ([page, artwork]) => {
    const bytes = Buffer.from(await artwork.arrayBuffer());
    const sha = crypto.createHash("sha256").update(bytes).digest("hex");
    if (page.status !== 200 || artwork.status !== 200 || sha !== "9490ea70b977d4d482c722d33ab8f9ee97df7ad6157cf63971d67afef598dd24") process.exit(70);
...
rollback() {
  ...
  printf '{"ok":false,"audit":"PROJECT_HAUL_ARTWORK_DEPLOY_ROLLED_BACK","stage":"%s","exitCode":%s}\n' "${stage}" "${rc}" >&2
  exit "${rc}"
}
trap 'rollback $?' ERR
...
test "$(curl --connect-timeout 5 --max-time 30 -fsS "https://www.logivya.com/logivya/project-haul-icon-transparent.png?release=${release}" | sha256sum | cut -d' ' -f1)" = "${artwork_sha}"
...
trap - ERR INT TERM HUP
activation_started=false
printf '{"ok":true,"audit":"PROJECT_HAUL_ARTWORK_PRODUCTION_DEPLOYED","release":"%s","webImage":"%s","sourceHash":"%s","patchSha256":"%s","artworkSha256":"%s","workerContainersUnchanged":true,"webCanary":true}\n' \
  "${release}" "${new_web_image}" "${source_hash}" "${expected_patch_sha}" "${artwork_sha}"

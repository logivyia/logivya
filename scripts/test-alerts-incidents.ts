...
const base = { environment: "test", service: "queue", type: "QUEUE_BACKLOG", windowMinutes: 15 };
...
assert.equal(first, duplicate, "Alerts in one cooldown window must deduplicate.");
assert.notEqual(first, later, "A later cooldown window must permit a new alert.");
...
const incidentRoute = readFileSync("src/app/api/admin/incidents/[id]/route.ts", "utf8");
...
assert(incidentRoute.includes("requirePlatformAdmin"));
...
process.stdout.write("Alert deduplication, cooldown and incident transition contracts passed.\n");

...
import { replaceAppStoreScreenshots } from "./replace-app-store-screenshots.mjs";
...
async function main() {
  ...
  if (activeSubmission.attributes?.state !== "WAITING_FOR_REVIEW") {
    throw new Error(`Review is ${activeSubmission.attributes?.state}; automatic replacement is not safe.`);
  }
  ...
  const replacement = await replaceAppStoreScreenshots({ configuration });
  const finalBuild = await verifyBuild(configuration, version.id);
  if (finalBuild.id !== initialBuild.id) throw new Error("Selected build changed during screenshot replacement.");
  ...
  console.log(JSON.stringify({
    ...
    screenshots: replacement.result.map((item) => ({

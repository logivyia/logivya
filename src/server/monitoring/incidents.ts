...
import { Prisma, type IncidentLog, type SecurityEventSeverity } from "@prisma/client";

import { sanitizeLogMetadata } from "@logivya/logging";
import { LOGIVYA_PLATFORM_OWNER_EMAIL } from "@/server/auth/platform-owner";
import { prisma } from "@/server/db";
import type { ServiceHealth, SystemHealthSnapshot } from "@/server/monitoring/contracts";
import { createNotification } from "@/server/notifications/service";

export const INCIDENT_STATUSES = ["OPEN", "ACKNOWLEDGED", "INVESTIGATING", "MITIGATED", "RESOLVED"] as const;
...
type IncidentMetadata = {
  ...
  timeline?: Array<{ at: string; action: string; actorUserId?: string; note?: string }>;
};

function record(value: Prisma.JsonValue | null): IncidentMetadata {
  return value && typeof value === "object" && !Array.isArray(value) ? value as IncidentMetadata : {};
}
...
function appendTimeline(metadata: IncidentMetadata, entry: NonNullable<IncidentMetadata["timeline"]>[number]) {
  return [...(metadata.timeline ?? []), entry].slice(-100);
}

async function notifyIncidentOpened(input: { incidentId: string; title: string; severity: string; service: string }) {
  const owner = await prisma.user.findUnique({
    where: { email: LOGIVYA_PLATFORM_OWNER_EMAIL },
    select: {
      id: true,
      ownedCompanies: { select: { id: true }, orderBy: { createdAt: "asc" }, take: 1 },
      memberships: { where: { status: "ACTIVE" }, select: { companyId: true }, orderBy: { createdAt: "asc" }, take: 1 },
    },
  });
  const companyId = owner?.ownedCompanies[0]?.id ?? owner?.memberships[0]?.companyId;
  ...
  await createNotification({
    ...
    payload: { incidentId: input.incidentId, severity: input.severity, service: input.service },
  });
}
...
export async function ensureIncidentForAlert(alert: {
  ...
  const id = incidentId(`alert:${alert.environment}:${alert.service}:${alert.type}`);
  ...
  if (!current || current.resolvedAt) await notifyIncidentOpened({ incidentId: incident.id, title: incident.title, severity: incident.severity, service: alert.service }).catch(() => undefined);
  return incident;
}

async function ensureHealthIncident(item: ServiceHealth, snapshot: SystemHealthSnapshot) {
  ...
  if (!current || current.resolvedAt) await notifyIncidentOpened({ incidentId: incident.id, title: incident.title, severity: incident.severity, service: item.id }).catch(() => undefined);
  ...
  await prisma.operationalAlert.upsert({
    ...
    create: {
      ...
      metadata: sanitizeLogMetadata({ safeErrorCode: item.safeErrorCode, runbook: item.runbook, incidentId: incident.id }) as Prisma.InputJsonValue,
    },
    update: {
      status: "OPEN",
      severity,
...
async function resolveRecoveredHealthIncident(item: ServiceHealth, snapshot: SystemHealthSnapshot) {
  ...
  const requiredChecks = Math.max(2, Number(process.env.MONITORING_RECOVERY_CONSECUTIVE_CHECKS || 2));
    ...
    await prisma.incidentLog.update({
      ...
      data: {
        metadata: sanitizeLogMetadata({
          ...metadata,
          recoveryCount,
          recoveryFirstSeenAt: metadata.recoveryFirstSeenAt ?? now.toISOString(),
          timeline: appendTimeline(metadata, { at: now.toISOString(), action: "RECOVERY_OBSERVED" }),
        }) as Prisma.InputJsonValue,
      },
    });
    return false;
    ...
    prisma.incidentLog.update({
      ...
      data: {
        ...
        metadata: sanitizeLogMetadata({
          ...metadata,
          recoveryCount,
          resolutionNote: "Automatically resolved after consecutive health evidence recovered.",
          timeline: appendTimeline(metadata, { at: now.toISOString(), action: "AUTO_RESOLVED" }),
        }) as Prisma.InputJsonValue,
      },
    }),
    prisma.operationalAlert.updateMany({
      where: { dedupeKey: `health:${snapshot.environment}:${item.id}`, status: { in: ["OPEN", "ACKNOWLEDGED"] } },
...
export async function reconcileHealthIncidents(snapshot: SystemHealthSnapshot) {
  ...
  for (const item of snapshot.services) {
    const actionable = item.state === "UNAVAILABLE" || (item.tier === 0 && ["DEGRADED", "UNKNOWN"].includes(item.state));
    if (actionable) {
      await ensureHealthIncident(item, snapshot);
      openedOrUpdated += 1;
    } else if (item.state === "HEALTHY" && await resolveRecoveredHealthIncident(item, snapshot)) {
      resolved += 1;
    }
  }
...
export async function updateIncident(input: {
  ...
  const currentStatus = INCIDENT_STATUSES.includes(input.incident.status as IncidentStatus)
  ...
  const nextMetadata: IncidentMetadata = {
    ...metadata,
    ...(input.status === "ACKNOWLEDGED" ? { acknowledgedAt: now.toISOString(), acknowledgedByUserId: input.actorUserId } : {}),
    ...(input.status === "RESOLVED" ? { resolutionNote: note, resolvedByUserId: input.actorUserId } : { investigationNote: note }),
    timeline: appendTimeline(metadata, { at: now.toISOString(), action: input.status, actorUserId: input.actorUserId, note }),
  };
  ...
  if (metadata.alertId || metadata.alertDedupeKey) {
    await prisma.operationalAlert.updateMany({
      where: metadata.alertId ? { id: metadata.alertId } : { dedupeKey: metadata.alertDedupeKey },
      data: input.status === "RESOLVED"
        ? { status: "RESOLVED", resolvedAt: now }
        : { status: "ACKNOWLEDGED", acknowledgedAt: now, acknowledgedByUserId: input.actorUserId },
    });
  }
  return updated;
}

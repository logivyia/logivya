...
export async function processNextTelegramDelivery(now = new Date()) {
  ...
  try {
    // Recheck at delivery time: a scheduled job must not outlive its sender's
    // membership, subscription, or the ownership of its selected account/chat.
    if (dispatch.account.ownerUserId !== dispatch.createdById || dispatch.account.companyId !== dispatch.companyId
      || delivery.chat.accountId !== dispatch.accountId || delivery.chat.companyId !== dispatch.companyId
      || dispatch.account.archivedAt || !delivery.chat.canSend || delivery.chat.isArchived || !delivery.chat.isActive) {
      throw new Error("TELEGRAM_SEND_FORBIDDEN");
    }
    await assertTelegramSendAccess({ userId: dispatch.createdById, companyId: dispatch.companyId, scheduleType: dispatch.scheduleType, chatTypes: [delivery.chat.type] });
    ...
    const nonRetryable = ["MESSAGE_ATTRIBUTION_LENGTH_EXCEEDED", "TELEGRAM_SEND_FORBIDDEN", "TELEGRAM_SUBSCRIPTION_LOCKED", "TELEGRAM_PLAN_FORBIDDEN", "TELEGRAM_VALIDATION_TARGETS"].includes(errorMessage);

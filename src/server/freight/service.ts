...
export async function getFreightListing(id: string, requesterUserId: string) {
  ...
  if (!row || ((row.status !== "ACTIVE" || (row.expiresAt && row.expiresAt <= new Date())) && row.ownerUserId !== requesterUserId)) {
    throw new Error("FREIGHT_LISTING_NOT_FOUND");
  }
  return serializeListingRow(row, await canReadMarketplaceContact(requesterUserId), row.ownerUserId === requesterUserId);
}

...
import { buildWhatsAppReferralMessage } from "@/server/freight/public-listing-summary";
...
export function boundedWebLimit(value: string | null | undefined, fallback = 100) {
  const parsed = Number(value ?? fallback);
  return Number.isInteger(parsed) ? Math.min(200, Math.max(1, parsed)) : fallback;
}
  ...
  return events.map((item) => {
    ...
    return {
      ...
      listing: {
        ...
        kind: item.listing.kind,
        href,
        publicTitle: cleanPublicText(item.listing.publicTitle) ?? "İlan",
        publicDescription: cleanPublicText(item.listing.publicDescription),
...
export async function serializeWebListingDetail(
  ...
  return {
    ...
    status,
    isActive: active,
    publicTitle: cleanPublicText(listing.publicTitle) ?? "İlan",
    publicDescription: cleanPublicText(listing.publicDescription),

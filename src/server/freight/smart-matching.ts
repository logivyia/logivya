...
async function dispatchDemandMatchSummary(demandId: string, triggerKey: string, jobId?: string) {
  const demand = await prisma.marketplaceDemandRequest.findUnique({
    where: { id: demandId },
    select: { id: true, companyId: true, ownerUserId: true, title: true, expiresAt: true, status: true, notificationsEnabled: true },
  });
  if (!demand || !demand.notificationsEnabled || demand.status !== "ACTIVE" || demand.expiresAt <= new Date()) return false;
  const [marketplaceMatches, smartMatches] = await Promise.all([
    prisma.marketplaceDemandMatch.findMany({ where: { requestId: demandId, notifiedAt: null, status: { not: "DISMISSED" } }, select: { id: true } }),
    prisma.smartMatchResult.findMany({ where: { demandId, notifiedAt: null, status: { in: ["NEW", "VIEWED", "SAVED"] } }, select: { id: true } }),
  ]);
  const count = marketplaceMatches.length + smartMatches.length;
  ...
  await prisma.$transaction([
    prisma.marketplaceDemandMatch.updateMany({
      where: { id: { in: marketplaceMatches.map(match => match.id) }, requestId: demandId, notifiedAt: null },
      data: { notifiedAt, notificationEventId: event.event.id },
    }),
    prisma.smartMatchResult.updateMany({ where: { id: { in: smartMatches.map(match => match.id) }, demandId, notifiedAt: null }, data: { notifiedAt } }),

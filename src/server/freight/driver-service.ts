...
export async function getDriverListing(id: string, requesterUserId: string) {
  ...
  if (!row || (row.status !== "ACTIVE" && row.ownerUserId !== requesterUserId)) throw new Error("DRIVER_LISTING_NOT_FOUND");
  return serializeDriverRow(row, true, row.ownerUserId === requesterUserId);
}

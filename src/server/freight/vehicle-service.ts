...
export async function getVehicleListing(id: string, requesterUserId: string) {
  ...
  if (!row || ((row.status !== "ACTIVE" || (row.expiresAt && row.expiresAt <= new Date())) && row.ownerUserId !== requesterUserId)) throw new Error("VEHICLE_LISTING_NOT_FOUND");
  return serializeVehicleRow(row, await canReadMarketplaceContact(requesterUserId), row.ownerUserId === requesterUserId);
}

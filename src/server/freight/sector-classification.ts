...
export function automaticListingExpiry(sourceTimestamp: Date, hasExplicitDate: boolean) {
  return hasExplicitDate ? null : new Date(sourceTimestamp.getTime() + automaticListingDefaultTtlHours() * 60 * 60_000);
}

export function automaticListingDefaultTtlHours() {
  const configured = Number(process.env.AUTO_LISTING_DEFAULT_TTL_HOURS || 48);
  return Number.isInteger(configured) && configured >= 1 && configured <= 720 ? configured : 48;
}

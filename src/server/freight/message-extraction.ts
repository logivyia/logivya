...
export function isProbableFreightMessage(text: string) {
  ...
  const locations = findLogisticsLocations(text);
...
export function extractFreightCandidates(text: string, timestamp: Date): ExtractedFreightCandidate[] {
  ...
  const sections = opportunitySections(text);
  const candidates: ExtractedFreightCandidate[] = [];
  for (const section of sections) {
    const locations = findLogisticsLocations(section);
    if (locations.length < 2 && !hasDriverSignal(section)) continue;
    const origin = locations[0] ?? null;
    const destination = locations.find((item) => item.position > (origin?.position ?? -1) && item.canonical !== origin?.canonical) ?? null;
    ...
    candidates.push({
      ...
      sourceExcerpt: section.slice(0, 1_500),
      confidence,
...
function opportunitySections(text: string) {
  const normalized = text.replace(/\r\n?/gu, "\n");
  const lines = normalized.split("\n").map((line) => line.trim()).filter(Boolean);
  const routeLines = lines.filter((line) => findLogisticsLocations(line).length >= 2);
  if (routeLines.length > 1) return routeLines;
  const bulletSections = normalized.split(/(?:\n\s*[-•▪◾✅☑️]+\s*|\n\s*\d+[.)]\s*)/gu).map((value) => value.trim()).filter(Boolean);
  const useful = bulletSections.filter((section) => findLogisticsLocations(section).length >= 2 || hasDriverSignal(section));
  return useful.length ? useful : [normalized.trim()];
}

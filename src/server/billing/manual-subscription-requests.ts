...
function throwCheckoutEligibilityError(input: {
  ...
  if (input.missingFields.length) {
    throw new ManualSubscriptionRequestError(input.missingFields[0], 400, {
...
function buyerSnapshot(
  ...
  const personalProfile =
    profile?.billingType === "INDIVIDUAL" ? profile : null;
  return {
...
function storedSeller(
  ...
  return {
    ...
    tradeRegistryNumber:
      typeof seller.tradeRegistryNumber === "string"
        ? seller.tradeRegistryNumber
        : null,
    mersisNumber:
      typeof seller.mersisNumber === "string" ? seller.mersisNumber : null,
  };
}

function legalDocumentsForRequest(
  request: {
    publicId: string;
...
export async function getBillingCheckoutConfiguration() {
  ...
  return {
    ...
    seller:
      configuration && state.checkoutAvailable
        ? publicSellerIdentity(configuration)
        : null,
  };
}
...
export async function createManualSubscriptionDraft(input: {
  ...
  const [persistedPlan, profile, sellerConfiguration] = await Promise.all([
    ...
    prisma.companyBillingProfile.findUnique({
      where: { companyId: input.context.company.id },
    }),
    prisma.billingSellerConfiguration.findUnique({ where: { id: "logivya" } }),
  ]);
  if (!persistedPlan?.isActive)
    throw new ManualSubscriptionRequestError("PLAN_NOT_FOUND", 404);
    ...
    request = await prisma.$transaction(async (tx) => {
      const created = await tx.subscriptionRequest.create({
        data: {
          ...
          idempotencyKeyHash,
          expiresAt: new Date(
            Date.now() + REQUEST_EXPIRY_DAYS * 24 * 60 * 60_000,
          ),
        },
      });
      await tx.subscriptionRequestTransition.create({
...
export async function submitManualSubscriptionRequest(input: {
  ...
  const submission = await prisma.$transaction(
    ...
    `;
      ...
      const consentLocale =
        input.request.headers.get("x-logivya-locale")?.slice(0, 16) ||
        input.request.headers
          .get("accept-language")
          ?.split(",")[0]
          ?.slice(0, 16) ||
        "tr";
      const persistCurrentConsents = () =>
        Promise.all(
          legalDocuments.map((document) =>
            tx.subscriptionRequestConsent.upsert({
              where: {
...
export async function listManualSubscriptionRequestsForCompany(
  companyId: string,
) {
...
export async function getManualSubscriptionRequestForCompany(
  id: string,
  companyId: string,
) {
  ...
  return prisma.$transaction(
    ...
    `;
      ...
      if (!CANCELLABLE_STATUSES.includes(request.status)) {
        throw new ManualSubscriptionRequestError(
          "SUBSCRIPTION_REQUEST_NOT_CANCELLABLE",
          409,
        );
      }
      const updated = await tx.subscriptionRequest.update({
...
export async function listManualSubscriptionRequestsForAdmin(input: {
  ...
  const searchableWhere: Prisma.SubscriptionRequestWhereInput = {
    status: { not: "DRAFT" },
    ...(query
      ? {
          OR: [
    ...
    `;
      ...
      const allowed =
        input.action === "UNDER_REVIEW"
          ? REVIEWABLE_STATUSES
          : input.action === "CLARIFICATION_REQUIRED" ||
      ...
      const updated = await tx.subscriptionRequest.update({
        ...
        data: {
          ...
          reviewedAt: now,
          adminCustomerNote:
            input.customerNote?.trim() || request.adminCustomerNote,
          adminInternalNote:
            input.internalNote?.trim() || request.adminInternalNote,
      ...
      if (request.requestedByUserId) {
        const notification = requestStatusNotification(
          input.action,
          request.planName,
        );
        await tx.notification.create({
...
function requestStatusNotification(
  status: SubscriptionRequestStatus,
  planName: string,
) {
  if (status === "UNDER_REVIEW") {
    return {
      type: "subscription.request_under_review",
      title: "Ödemeniz kontrol ediliyor",
      message: "Ödemeniz kontrol ediliyor.",
    };
  }
  if (status === "CLARIFICATION_REQUIRED") {
    return {
      type: "subscription.request_clarification",
      title: "Ek bilgi gerekiyor",
      message: "Ödemenizin eşleştirilmesi için ek bilgi gerekiyor.",
    };
  }
  if (status === "REJECTED") {
    return {
      type: "subscription.request_rejected",
      title: "Abonelik talebi onaylanamadı",
      message:
        "Abonelik talebiniz onaylanamadı. Ayrıntıları abonelik sayfasından inceleyebilirsiniz.",
    };
  }
  if (status === "CANCELLED") {
    return {
  ...
  return {
    type: "subscription.request_activated",
    title: "Aboneliğiniz etkinleştirildi",
    message: `Ödemeniz onaylandı. ${planName} paketiniz etkinleştirildi.`,
  };
}
...
export function serializeManualSubscriptionRequest(
  ...
  return {
    ...
    legalConsentSnapshot: legalDocuments.map((document) => ({
      ...
      hash: document.hash,
      acceptedAt:
        "acceptedAt" in document && typeof document.acceptedAt === "string"
          ? document.acceptedAt
          : null,
    })),
    resultingSubscriptionId: request.activationSubscriptionId,
    rejectionReason:
      request.status === "REJECTED" ? request.adminCustomerNote : null,
    duplicate,
...
function serializeDetailedManualSubscriptionRequest(
  request: RequestWithDetails | (RequestWithDetails & { company: unknown }),
) {
  const legalDocuments = request.consents.map((consent) => {
    const snapshot = storedObject(
      consent.documentSnapshot,
      "INVALID_LEGAL_DOCUMENT_SNAPSHOT",
    );
    return {
...
export function manualSubscriptionRequestStatus(error: unknown) {
  ...
  if (error.message === "FORBIDDEN" || error.message === "CSRF_REJECTED")
    return 403;
...
export function manualSubscriptionRequestErrorBody(error: unknown) {
  ...
  const code =
    error instanceof Error ? error.message : "SUBSCRIPTION_REQUEST_FAILED";
  return {
    error: [
      "UNAUTHORIZED",
      "FORBIDDEN",

...
export function isFreePromotionalPayment(
  payment: CompanySubscriptionActivationInput["payment"],
) {
  return Boolean(
    payment?.mode === "CREATE"
      && payment.provider === "MANUAL"
      && payment.paymentMethod === "FREE_PROMO",
  );
}
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (
        ...
        && !isFreePromotionalPayment(input.payment)
      ...
      if (
        payment
        && profile
        && !isStoreBillingProvider(payment.provider)
        && !isFreePromotionalPayment(input.payment)
      ) {
      ...
      await tx.subscriptionEvent.create({
        data: {
          ...
          metadata: {
            ...
            usedSeats,
            targetSeatLimit,
            freePromotionalGrant: isFreePromotionalPayment(input.payment),
          },
        },
      });
      if (payment && !isFreePromotionalPayment(input.payment)) {
        await tx.subscriptionEvent.create({
      ...
      await tx.notification.create({
        data: {
          ...
          message: manualRequest
            ...
            : isFreePromotionalPayment(input.payment)
              ? `${plan.name} paketiniz ${input.endsAt.toLocaleDateString("tr-TR")} tarihine kadar yönetici hediyesi olarak aktif.`

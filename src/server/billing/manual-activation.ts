...
export type ManualActivationInput={companyId:string;planSlug:string;billingPeriod:"MONTHLY"|"YEARLY"|"CUSTOM";startsAt:Date;endsAt:Date;currency:string;paymentMethod:"MANUAL_BANK_TRANSFER"|"CREDIT_CARD"|"DEBIT_CARD"|"CASH"|"OTHER";adminUserId:string;note?:string;idempotencyKey:string;customAmount?:number};
export async function activateSubscriptionManually(input:ManualActivationInput){
  ...
  const amount=input.billingPeriod==="YEARLY"?plan.yearlyPrice:input.billingPeriod==="MONTHLY"?plan.monthlyPrice:new Prisma.Decimal(input.customAmount??0);
  return prisma.$transaction(async tx=>{
    ...
    const payment=await tx.payment.create({data:{companyId:company.id,subscriptionId:subscription.id,planId:plan.id,provider:"MANUAL",providerPaymentId,status:"PAID",paymentMethod:input.paymentMethod,amount,currency:input.currency,paidAt:new Date(),metadata:{note:input.note??""}}});

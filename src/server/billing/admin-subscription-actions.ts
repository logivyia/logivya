...
export type AdminSubscriptionAction =
  ...
  | { action: "CHANGE_PLAN"; reason: string; planSlug: string };

export class AdminSubscriptionActionError extends Error {
  constructor(public readonly code: "NOT_FOUND" | "INVALID_EXTENSION") {
    super(code);
...
export async function performAdminSubscriptionAction(input: {
  ...
  if (data.action === "ACTIVATE" || data.action === "CHANGE_PLAN") {
    ...
    const endsAt = currentEnd && currentEnd > now ? currentEnd : new Date(now.getTime() + 30 * DAY_MS);
    ...
    return { before, subscription: result.subscription, activationManagedAudit: true };
  }
  ...
  if (data.action === "SUSPEND") {
    ...
    update = { status: "CANCELED", cancelledAt: new Date(), cancelAtPeriodEnd: false };
  } else {
    if (!data.endsAt && !data.extensionDays) throw new AdminSubscriptionActionError("INVALID_EXTENSION");
    const now = new Date();
    const existingEnd = before.currentPeriodEndsAt ?? before.endsAt ?? before.trialEndsAt;
    ...
    const endsAt = data.extensionDays
      ...
      : data.endsAt!;
    if (endsAt <= extensionBase) throw new AdminSubscriptionActionError("INVALID_EXTENSION");
    update = { status: "ACTIVE", endsAt, currentPeriodEndsAt: endsAt, expiredAt: null };
  }

  const subscription = await prisma.$transaction(async (tx) => {
    const changed = await tx.subscription.update({ where: { id: input.subscriptionId }, data: update });

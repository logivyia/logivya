import {
  Environment,
  SignedDataVerifier,
  type JWSTransactionDecodedPayload,
  type ResponseBodyV2DecodedPayload,
} from "@apple/app-store-server-library";

import { APPLE_APP_BUNDLE_ID, APPLE_APP_ID } from "@/server/billing/apple-store-products";

// Public Apple Root CA certificates. SHA-256 fingerprints:
// G2 C2B9B042DD57830E7D117DAC55AC8AE19407D38E41D88F3215BC3A890444A050
// G3 63343ABFB89A6A03EBB57E9B3F5FA7BE7C4F5C756F3017B3A8C488C3653E9179
const APPLE_ROOT_CA_G2 = "";
const APPLE_ROOT_CA_G3 = "";

const roots = [APPLE_ROOT_CA_G2, APPLE_ROOT_CA_G3].map((value) => Buffer.from(value, "base64"));
const onlineChecks = process.env.APPLE_IAP_ONLINE_CHECKS !== "false";

const productionVerifier = new SignedDataVerifier(
  roots,
  onlineChecks,
  Environment.PRODUCTION,
  APPLE_APP_BUNDLE_ID,
  Number(process.env.APPLE_APP_ID || APPLE_APP_ID),
);
const sandboxVerifier = new SignedDataVerifier(
  roots,
  onlineChecks,
  Environment.SANDBOX,
  APPLE_APP_BUNDLE_ID,
);

async function verifyInEitherEnvironment<T>(
  operation: (verifier: SignedDataVerifier) => Promise<T>,
) {
  try {
    return await operation(productionVerifier);
  } catch (productionError) {
    try {
      return await operation(sandboxVerifier);
    } catch {
      throw productionError;
    }
  }
}

export function verifyAppleSignedTransaction(
  signedTransactionInfo: string,
): Promise<JWSTransactionDecodedPayload> {
  return verifyInEitherEnvironment((verifier) =>
    verifier.verifyAndDecodeTransaction(signedTransactionInfo),
  );
}

export function verifyAppleNotification(
  signedPayload: string,
): Promise<ResponseBodyV2DecodedPayload> {
  return verifyInEitherEnvironment((verifier) =>
    verifier.verifyAndDecodeNotification(signedPayload),
  );
}

export function verifyAppleRenewalInfo(signedRenewalInfo: string) {
  return verifyInEitherEnvironment((verifier) =>
    verifier.verifyAndDecodeRenewalInfo(signedRenewalInfo),
  );
}

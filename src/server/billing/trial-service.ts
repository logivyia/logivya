...
const IDENTITY_CONSUMING_STATUSES = [
  "ACTIVE",
  "CONSUMED",
  "PAID_USAGE",
] as const;
...
export function normalizeVerifiedPhone(
  value: string,
  defaultCountryCallingCode = "90",
) {
  let digits = value.replace(/\D/gu, "");
  if (digits.startsWith("00")) digits = digits.slice(2);
  if (digits.startsWith("0") && digits.length === 11)
    digits = `${defaultCountryCallingCode}${digits.slice(1)}`;
  if (!digits.startsWith(defaultCountryCallingCode) && digits.length === 10)
    digits = `${defaultCountryCallingCode}${digits}`;
  if (digits.length < 8 || digits.length > 15)
    throw new Error("VERIFIED_WHATSAPP_PHONE_INVALID");
  return `+${digits}`;
}
...
function normalizeWhatsAppIdentity(value: string) {
  ...
  if (!normalized || normalized.length > 200)
    throw new Error("VERIFIED_WHATSAPP_IDENTITY_INVALID");
  return normalized;
}

export function createPendingTrialEntitlement(
  tx: Prisma.TransactionClient,
  input: PendingTrialInput,
) {
  return tx.trialEntitlement.create({
    data: {
      ...
      registrationPhoneHash: optionalHash(
        "registration-phone",
        input.registrationPhone,
      ),
      registrationIpHash: optionalHash("registration-ip", input.ipAddress),
      deviceFingerprintHash: optionalHash(
        "registration-device",
...
async function supportingRisk(
  ...
  const registrationPhoneMatches = candidate.registrationPhoneHash
    ? await tx.trialEntitlement.count({
        where: {
          id: { not: candidate.id },
          registrationPhoneHash: candidate.registrationPhoneHash,
          status,
        },
      })
    : 0;
  const registrationIpMatches = candidate.registrationIpHash
    ? await tx.trialEntitlement.count({
        where: {
          id: { not: candidate.id },
          registrationIpHash: candidate.registrationIpHash,
          status,
        },
      })
    : 0;
  const deviceFingerprintMatches = candidate.deviceFingerprintHash
    ? await tx.trialEntitlement.count({
        where: {
          id: { not: candidate.id },
          deviceFingerprintHash: candidate.deviceFingerprintHash,
          status,
        },
      })
    : 0;
  const checks = [
    registrationPhoneMatches,
    registrationIpMatches,
    deviceFingerprintMatches,
  ];
  if (checks[0]) {
    score += 25;
    signals.push("REGISTRATION_PHONE_REUSED");
  }
  if (checks[1]) {
    score += 15;
    signals.push("REGISTRATION_NETWORK_REUSED");
  }
  if (checks[2]) {
    score += 35;
    signals.push("DEVICE_FINGERPRINT_REUSED");
  }
...
async function recordPaidIdentity(
  ...
  const decisionCode = duplicate
    ? "IDENTITY_ALREADY_RECORDED"
    : "PAID_IDENTITY_RECORDED";
...
export async function activateTrialAfterVerifiedWhatsAppConnection(
  accountId: string,
) {
  ...
  const connectedAt = account.pairedAt ?? account.lastConnectedAt ?? new Date();

  return prisma.$transaction(
    async (tx) => {
      await acquireIdentityLocks(tx, [
        `trial-owner:${account.companyId}:${account.userId}`,
        phoneHash,
        identityHash,
      ]);
      ...
      const candidate = await tx.trialEntitlement.findUnique({
        where: {
          companyId_userId: {
      ...
      if (current?.valid && current.subscription.source === "TRIAL") {
        if (!candidate)
          return { outcome: "ACTIVE_TRIAL_WITHOUT_ENTITLEMENT" as const };
      ...
      if (
        !candidate ||
        candidate.status !== "PENDING_IDENTITY" ||
        company.ownerId !== account.userId
      ) {
      ...
      if (
        duplicate ||
        (risk.score >= 70 &&
          candidate.decisionCode !== "MANUAL_REVIEW_APPROVED")
      ) {
      ...
      await tx.subscriptionEvent.create({
        data: {
          ...
          type: "TRIAL_STARTED",
          message:
            "7 günlük ücretsiz deneme, doğrulanmış WhatsApp bağlantısı ile başlatıldı.",
        },
      });
      await tx.subscriptionAuditLog.create({
        data: {
      ...
      await tx.notification.create({
        data: {
          ...
          message:
            "7 günlük ücretsiz denemeniz WhatsApp bağlantınız doğrulandıktan sonra başladı.",
        },
      });
...
export async function safelyEvaluateTrialAfterConnection(accountId: string) {
  for (let attempt = 1; attempt <= TRIAL_TRANSACTION_ATTEMPTS; attempt += 1) {
    try {
      const result =
        await activateTrialAfterVerifiedWhatsAppConnection(accountId);
      logger.info("trial.identity_evaluated", {
        accountId,
      ...
      if (
        isRetryableTrialTransactionError(error) &&
        attempt < TRIAL_TRANSACTION_ATTEMPTS
      ) {
        logger.warn("trial.identity_evaluation_retry", { accountId, attempt });
        continue;
      }
      logger.error("trial.identity_evaluation_failed", error, {
        accountId,
        attempt,
      });
...
export type ConnectedPendingTrialCandidate = {
...
export async function findConnectedPendingTrialCandidates(limit = 100) {
  const take = Math.min(Math.max(Math.trunc(limit), 1), 500);
  const pending = await prisma.trialEntitlement.findMany({
  ...
  const linkedIds = pending.flatMap((candidate) =>
    candidate.whatsappAccountId ? [candidate.whatsappAccountId] : [],
  );
  const ownerPairs = new Map<string, { companyId: string; userId: string }>();
  for (const candidate of pending) {
    ownerPairs.set(`${candidate.companyId}:${candidate.userId}`, {
  ...
  const accountsByOwner = new Map<string, (typeof connectedAccounts)[number]>();
  for (const account of connectedAccounts) {
    if (!account.userId) continue;
    const ownerKey = `${account.companyId}:${account.userId}`;
    if (!accountsByOwner.has(ownerKey)) accountsByOwner.set(ownerKey, account);
  }

  const candidates: ConnectedPendingTrialCandidate[] = [];
  for (const pendingCandidate of pending) {
    const linkedAccount = pendingCandidate.whatsappAccountId
      ? accountsById.get(pendingCandidate.whatsappAccountId)
      : undefined;
    const linkedAccountOwnedByCandidate =
      linkedAccount?.companyId === pendingCandidate.companyId &&
      linkedAccount.userId === pendingCandidate.userId
        ? linkedAccount
        : undefined;
    const account =
      linkedAccountOwnedByCandidate ??
      accountsByOwner.get(
        `${pendingCandidate.companyId}:${pendingCandidate.userId}`,
      );
    if (!account) continue;

    candidates.push({
      entitlementId: pendingCandidate.id,
      companyId: pendingCandidate.companyId,
      userId: pendingCandidate.userId,
      accountId: account.id,
      linked: pendingCandidate.whatsappAccountId === account.id,
      decisionCode: pendingCandidate.decisionCode,
...
export async function reconcileConnectedPendingTrials(limit = 100) {
  ...
  for (const candidate of candidates) {
    const result = await safelyEvaluateTrialAfterConnection(candidate.accountId);
    results.push({

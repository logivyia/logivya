...
export async function enforceWhatsAppIngestionRetention() {
  const now = new Date();
  const today = todayFreightDate(now);
  const driverCutoff = new Date(now.getTime() - 7 * 86_400_000);
  ...
  const [rawMessages, attachments, expiredReview, expiredLoads, expiredVehicles, expiredDrivers] = await prisma.$transaction([
    prisma.whatsAppInboundMessage.updateMany({
      ...
      data: { rawTextEncrypted: null, normalizedText: null },
    }),
    ...
    prisma.freightListing.updateMany({
      where: { source: "WHATSAPP", status: "ACTIVE", OR: [{ loadingDate: { lt: today } }, { expiresAt: { lte: now } }] },
      data: { status: "INACTIVE", deactivatedAt: now },
    }),
    prisma.vehicleListing.updateMany({
      where: { source: "WHATSAPP", status: "ACTIVE", OR: [{ availableUntil: { lt: today } }, { expiresAt: { lte: now } }] },
      data: { status: "INACTIVE", deactivatedAt: now },
    }),
    prisma.driverListing.updateMany({
      where: { source: "WHATSAPP", status: "ACTIVE", OR: [{ updatedAt: { lt: driverCutoff } }, { expiresAt: { lte: now } }] },
      data: { status: "INACTIVE", deactivatedAt: now },
    }),
  ]);
  return {
...
async function classifyContent(job: WhatsAppIngestionJob): Promise<StageResult> {
  ...
  await persistAiExtractions(row.id, logisticsListings, aiResult.model, row.group.sectorHint);
...
async function extractStructuredListings(job: WhatsAppIngestionJob): Promise<StageResult> {
  ...
  const partial = /(?:parsiyel|kısmi|kismi|partial|сборн|qisman|qismən|جزئي|خرده\s*بار)/iu.test(raw);
  for (const candidate of candidates) {
    ...
    const structuredData = {
      ...
      normalizedDescription: raw.trim().slice(0, 2_000),
      ...
      extractionModel: "logivya-local-rules-v2",
    ...
    await prisma.whatsAppListingExtraction.upsert({
      ...
      create: {
        ...
        freightCurrency: candidate.currency,
        normalizedPhoneEncrypted: candidate.advertisedBusinessContactEncrypted,
        companyName: candidate.companyName,
        confidenceScore: candidate.extractionConfidence,
        missingCriticalFields,
        extractedFromMedia: row.attachments.length > 0,
...
async function persistAiExtractions(
  ...
  for (const [index, listing] of listings.entries()) {
    ...
    const normalizedPhoneEncrypted = listing.contactPhone ? encryptPrivateValue(listing.contactPhone) : null;
    const structuredData = {
      ...listing,
...
async function publishExtraction(input: {
  ...
  const description = extraction.normalizedDescription?.slice(0, 2_000) || null;
  ...
  if (extraction.listingType === "DRIVER") {
    ...
    const title = extraction.title?.slice(0, 140) || `Şoför · ${extraction.originCity}`;
    const common = {
      listingType,
      title,
...
function detectLanguage(value: string) {
  if (/\p{Script=Arabic}/u.test(value)) return "fa-ar";
...
export function extractionEngineAllowsAutomaticPublication(value: Prisma.JsonValue) {
  ...
  return engine === "LOGIVYA_LOCAL_RULE_ENGINE"
    && structuredString(value, "extractionModel") === "logivya-local-rules-v2";
}

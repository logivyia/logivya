...
export function webCommunicationSafeError(error: unknown) {
  ...
  if (code === "TELEGRAM_SUBSCRIPTION_LOCKED" || code === "TELEGRAM_PLAN_FORBIDDEN") {
    return NextResponse.json({ ok: false, error: code, message: "Bu gönderim için paketinizin aktif olması ve gerekli mesajlaşma özelliğini içermesi gerekir." }, { status: 403 });
  }

...
import type { PublicProductPage as PublicProductPageContent } from "@/config/public-product-pages";
import { arabicPublicProductPage } from "@/config/public-product-pages-ar";
...
export async function generateMetadata({ params }: { params: Promise<{ publicSlug: string }> }): Promise<Metadata> {
  ...
  const configuredPage = PUBLIC_PRODUCT_PAGE_MAP.get(publicSlug);
  if (!configuredPage) return {};
  const { locale, t } = await getServerTranslator();
  const page = localizedPublicProductPage(configuredPage, locale, t);
...
function publicStructuredData(page: (typeof PUBLIC_PRODUCT_PAGES)[number], unavailable: boolean) {
  const graph: Array<Record<string, unknown>> = [
    { "@type": "Organization", "@id": `${SITE_URL}/#organization`, name: "Logivya", url: SITE_URL, logo: `${SITE_URL}/android-chrome-512x512.png?v=5` },
...
function localizedPublicProductPage(
  page: PublicProductPageContent,
  locale: Locale,
  t: (key: string, params?: Record<string, string | number>) => string,
): PublicProductPageContent {
  if (locale === "ar") return arabicPublicProductPage(page);
  if (page.slug !== "logivya-nedir" && page.slug !== "sss") return page;

  const prefix = page.slug === "logivya-nedir" ? "publicAbout" : "publicFaq";
  return {
    ...page,
    title: t(`${prefix}.title`),
    description: t(`${prefix}.description`),
    eyebrow: t(`${prefix}.eyebrow`),
    paragraphs: [t(`${prefix}.paragraph1`), t(`${prefix}.paragraph2`), t(`${prefix}.paragraph3`)],
    bullets: [t(`${prefix}.bullet1`), t(`${prefix}.bullet2`), t(`${prefix}.bullet3`), t(`${prefix}.bullet4`)],
    audience: t(`${prefix}.audience`),
    howItWorks: [t(`${prefix}.how1`), t(`${prefix}.how2`)],
    useCases: [t(`${prefix}.use1`), t(`${prefix}.use2`), t(`${prefix}.use3`)],
    limitations: [t(`${prefix}.limitation1`), t(`${prefix}.limitation2`)],
  };
}

...
export async function POST(request: Request) {
  ...
  if (isPlatformAdmin) {
    const [trusted, admin] = await Promise.all([
      prisma.trustedDevice.findFirst({ where: { userId: user.id, ipAddress, revokedAt: null } }),
    ...
    if (riskScore > 20) {
      await prisma.securityEvent.create({
        data: {
          userId: user.id,
          severity: "MEDIUM",
          type: "ADMIN_NEW_DEVICE_LOGIN",
          message: "Platform yöneticisi yeni IP veya cihazdan giriş yaptı.",
          ipAddress,
          userAgent,
          metadata: { riskScore },
        },

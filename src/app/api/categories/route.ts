...
export async function GET() {
  try {
    ...
    const categories = await prisma.category.findMany({ where: { companyId: company.id, archivedAt: null }, include: { _count: { select: { groups: true } }, groups: { include: { group: { select: { id: true, name: true, canSend: true, isArchived: true } } } } }, orderBy: { name: "asc" }, take: 200 });

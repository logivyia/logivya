...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const { user, company } = await requireCriticalAdminAction(
      request,
      "admin.subscriptions.approve",
      parsed.data.reason,
    );
    const { confirmVerifiedIdentity, confirmLegalApproval, reason, ...fields } =
      parsed.data;
    const before = await prisma.billingSellerConfiguration.findUnique({
      where: { id: "logivya" },
    });
    const configuration = await prisma.billingSellerConfiguration.upsert({
      ...
      update: {
        ...fields,
        verifiedAt: confirmVerifiedIdentity
          ? before?.verifiedAt || new Date()
          : null,
        legalDocumentsApprovedAt: confirmLegalApproval
          ? before?.legalDocumentsApprovedAt || new Date()
          : null,
        updatedByUserId: user.id,
      },
    });
    ...
    await writeAuditLog(request, {
      ...
      before: before
        ? {
            complete: billingSellerConfigurationState(before).checkoutAvailable,
          }
        : {},
      after: {

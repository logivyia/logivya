...
export async function POST(
  ...
  try {
    ...
    if (!["AWAITING_PAYMENT", "UNDER_REVIEW", "CLARIFICATION_REQUIRED"].includes(subscriptionRequest.status)) {
    ...
    const samePlanRenewal = current?.plan.slug.toUpperCase() === subscriptionRequest.planCode
      && currentEnd
      && currentEnd > new Date();
    const approvalTime = new Date();
    ...
    if (error instanceof SubscriptionActivationError) {
      const status = error.message === "SUBSCRIPTION_REQUEST_STATE_CONFLICT" ? 409 : 400;
      return NextResponse.json({
        error: error.message,
        details: error.details,
        requestId: correlationId,

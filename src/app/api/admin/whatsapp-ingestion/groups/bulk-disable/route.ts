  ...
  try {
    ...
    if (!parsed.success)
      return NextResponse.json(
        {
          error: "WHATSAPP_INGESTION_BULK_DISABLE_INVALID",
    ...
    const result = await bulkUpdateWhatsAppIngestionGroups({
      ...parsed.data,
      enabled: false,
    ...
    await writeAuditLog(request, {
      ...
      action: "whatsapp.ingestion_sources.bulk_disabled",
      entityType: "WhatsAppGroup",
      entityId: "bulk",
      reason: parsed.data.reason,
      metadata: { ids: parsed.data.ids, updated: result.updated },
      requestId: id,
    });

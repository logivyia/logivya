...
export async function GET(request: Request) {
  try {
    ...
    const [
      ...
      prisma.messageRecipient.count({
        where: { status: "FAILED", failedAt: { gte: today } },
      }),
      ...
      prisma.securityEvent.count({
        where: { result: "DENIED", createdAt: { gte: today } },
      }),
      prisma.auditLog.count({
        where: { actorType: "PLATFORM_ADMIN", createdAt: { gte: today } },
      }),
      prisma.supportTicket.count({ where: { createdAt: { gte: today } } }),
      prisma.contactSyncRun.count({
      ...
      prisma.securityEvent.count({
        where: { type: "CLIENT_ERROR_REPORTED", createdAt: { gte: today } },
      }),

...
}).strict().refine((value) => Object.keys(value).some((key) => !["confirmation", "reason"].includes(key)), {
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const admin = await requireCriticalAdminAction(request, "admin.featureFlags.update", parsed.data.reason);

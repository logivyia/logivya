...
export async function POST(
  ...
  try {
    ...
    if (
      required.some(
        (key) => resolveTemplateVariable(variables, key) === undefined,
      )
    )
      return NextResponse.json(
        { error: "NOTIFICATION_TEMPLATE_VARIABLE_MISSING" },
        { status: 400 },
      );
    const content = {
      title: renderText(
        template.title || template.subject || "Logivya",
        variables,
      ),
      subject: renderText(
        template.subject || template.title || "Logivya",
        variables,
      ),
      message: renderText(template.body, variables),
    };
    let result: unknown;
    if (template.channel === "EMAIL")
      result = await sendTemplateEmailSafely({
        to: admin.user.email,
      ...
      template.channel === "ANDROID_PUSH" ||
      template.channel === "IOS_PUSH"
    )
      result = await sendPushToUserStrict({
        companyId: admin.company.id,
        userId: admin.user.id,
        title: content.title,
        message: content.message,
        type: template.eventType,
        notificationId: `template-test:${template.id}`,
        platform: template.channel === "ANDROID_PUSH" ? "ANDROID" : "IOS",
      });
    else if (template.channel === "WEB_PUSH")
    ...
    await writeAuditLog(request, {
      ...
      action: "notification.template.test_sent",
      ...
      metadata: {
        ...
        locale: template.locale,
        recipient: "AUTHORIZED_ADMIN_SELF",
      },
    });

...
export async function GET(request: Request) {
  try {
    ...
    const severity =
      params.get("severity")?.trim().toUpperCase() ||
      (severities.has(rawStatus) ? rawStatus : "");
    const status = statuses.has(rawStatus)
      ? rawStatus
      : params.get("eventStatus")?.trim().toUpperCase() || "";
    ...
    const [
      ...
      prisma.loginAttempt.count({
        where: { success: false, createdAt: { gte: since24Hours } },
      }),
      prisma.rateLimitEvent.count({
        where: { blocked: true, createdAt: { gte: since24Hours } },
      }),
      ...
      prisma.securityEvent.count({
        where: {
          createdAt: { gte: since24Hours },
          OR: [
            { type: { contains: "TENANT", mode: "insensitive" } },
            { type: { contains: "IDOR", mode: "insensitive" } },
          ],
        },
      }),
    ...
    void writeAuditLog(request, {
      ...
      logger.error("audit.admin_security_access.write_failed", error, {
        userId: context.user.id,
      }),
    );
      ...
      events: events.map((event) => ({
        ...
        user: event.user
          ? {
              ...
              emailMasked: maskEmail(event.user.email),
            }
          : null,
      })),
      metrics: {

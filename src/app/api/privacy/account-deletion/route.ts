...
const createSchema = z.object({
  ...
  confirmation: z.enum(["LOGIVYA HESABIMI SIL", "DELETE MY LOGIVYA ACCOUNT", "LOGIVYA CALISMA ALANIMI SIL", "DELETE MY LOGIVYA WORKSPACE"]),
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({
      ...
      isTenantOwner: auth.membership.role === "OWNER",
      deletionScope: auth.membership.role === "OWNER" ? "TENANT_OR_USER" : "MEMBERSHIP_ONLY",
      jobs,
    });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const companyConfirmation = ["LOGIVYA CALISMA ALANIMI SIL", "DELETE MY LOGIVYA WORKSPACE"].includes(parsed.data.confirmation);
    ...
    if (auth.membership.role !== "OWNER") {
      ...
      const job = await closeSharedMembership({
    ...
    const job = await queueDeletionRequest({ companyId: auth.company.id, userId: auth.user.id, scope: parsed.data.scope, owner: true, reason: parsed.data.reason });

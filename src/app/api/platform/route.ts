...
export async function GET() {
  try {
    ...
    const [accounts, groups, categories, campaigns, subscription] = await Promise.all([
      ...
      prisma.whatsAppGroup.findMany({ where: { companyId: company.id, isArchived: false }, include: { account: { select: { label: true } }, categories: { include: { category: true } } }, orderBy: { name: "asc" } }),
      prisma.category.findMany({ where: { companyId: company.id, archivedAt: null }, include: { _count: { select: { groups: true } } }, orderBy: { name: "asc" } }),

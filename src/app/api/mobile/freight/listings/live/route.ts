...
export async function GET(request: Request) {
  try {
    ...
    if (!wantsStream) {
      const events = await readLiveMarketplaceEvents(after, Number(url.searchParams.get("limit") || 1000), context.user.id, !afterValue, scope, { filters: parseMarketplaceFilters(url.searchParams) });

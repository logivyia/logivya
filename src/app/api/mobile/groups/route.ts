...
export async function GET(request: Request) {
  try {
    ...
    const accounts = await listRecoverableWhatsAppAccounts(scope, { accountId });
    ...
    const rows = await prisma.whatsAppGroup.findMany({
      ...
      select: {
        ...
        createdAt: true,
        account: {
          select: { status: true, archivedAt: true, lastError: true },
        },
        categories: {
    ...
    const start = cursor
      ? Math.max(
          0,
          visibleRows.findIndex((group) => group.id === cursor) + 1,
        )
      : 0;
    ...
    logger.info("whatsapp.group_listing.mobile", {
      userId: user.id,
      companyId: company.id,
      whatsappAccountIds: accountIds,
      returnedGroupCount: groups.length,
    });
    return mobileSuccess({
      groups,
      pageInfo: {
        nextCursor: hasMore ? groups.at(-1)?.id : null,
        hasMore,
      },
    });
  } catch (error) {

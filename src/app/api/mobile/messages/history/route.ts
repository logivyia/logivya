...
export async function GET(request: Request) {
  try {
    ...
    const rows = await prisma.messageCampaign.findMany({
      ...
      select: { id: true, title: true, content: true, status: true, scheduleType: true, scheduledAt: true, totalRecipients: true, sentCount: true, failedCount: true, createdAt: true },
      orderBy: { createdAt: "desc" },
      ...(cursor ? { cursor: { id: cursor }, skip: 1 } : {}),
      take: take + 1,
    });
    const hasMore = rows.length > take;
    const campaigns = rows.slice(0, take);

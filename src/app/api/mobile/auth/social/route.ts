  ...
  .superRefine((value, context) => {
    if (value.provider === "APPLE" && !value.nonce) {
      context.addIssue({
        code: "custom",
        path: ["nonce"],
        message: "Apple nonce is required.",
      });
    }
  });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    enforceMobileRateLimit(
      `mobile-social-login:${parsed.data.provider}:${ip}`,
      20,
      60 * 60_000,
    );
    ...
    if (!user || user.status !== "ACTIVE") {
      await prisma.loginAttempt.create({
        data: {
          ...
          success: false,
          failureReason: user
            ? "MOBILE_SOCIAL_ACCOUNT_DISABLED"
            : "MOBILE_SOCIAL_ACCOUNT_NOT_FOUND",
        },
      });
      await tryRecordSecurityEvent({
        ...
        metadata: {
          ...
          identityHash: keyedIdentifierHash(
      ...
      if (user) {
        return withAuthHeaders(
          await mobileError("FORBIDDEN", "api.error.forbidden", {
            status: 403,
          }),
        );
      }
    ...
    const membership = await resolvePreferredLoginMembership(
      user.id,
      parsed.data.deviceId,
    );
    if (!membership) {
      return withAuthHeaders(
        await mobileError("FORBIDDEN", "api.error.workspaceNotFound", {
          status: 403,
        }),
      );
    }
    ...
    const trustedDevice = mfa.enabledMethods.length
      ? await validateTrustedDevice(
          user.id,
          parsed.data.trustedDeviceToken,
          parsed.data.deviceId,
        )
      : null;

    if (mfa.mfaRequired && !trustedDevice) {
      const purpose = mfa.setupRequired
        ? ("SETUP" as const)
        : ("LOGIN" as const);
      const selectedMethod =
        purpose === "SETUP"
          ? mfa.requiredEnrollmentMethods.length === 1
            ? mfa.requiredEnrollmentMethods[0]
            : null
          : mfa.selectedMethod;
      ...
      const enrollment =
        purpose === "SETUP" && selectedMethod === "TOTP"
          ? await createAndStoreMfaEnrollment(user.id, user.email, {
              replacePending: true,
            })
          : null;
      const email =
        purpose === "LOGIN" && selectedMethod === "EMAIL_OTP"
          ? await sendEmailOtpForChallenge({
      ...
      await recordMfaSecurityEvent({
        ...
        companyId: membership.companyId,
        type:
          purpose === "SETUP" ? "MFA_SETUP_REQUIRED" : "MFA_CHALLENGE_ISSUED",
        message:
          purpose === "SETUP"
            ? "Mobil MFA kurulumu istendi."
            : "Mobil MFA kodu istendi.",
      });
      return withAuthHeaders(
        await mobileSuccess({
    ...
    if (error instanceof SocialIdentityError) {
      ...
      return withAuthHeaders(
        await mobileError(
          error.code,
          unavailable
            ? "api.error.socialLoginNotConfigured"
            : "api.error.socialTokenInvalid",
    ...
    if (error instanceof Error && error.message === "RATE_LIMITED") {
      return withAuthHeaders(
        await mobileError("RATE_LIMITED", "api.error.rateLimited", {
          status: 429,
        }),
      );
    }
    if (
      ...
      error.message === "MOBILE_AUTH_SECRET_MISSING"
    ) {
      return withAuthHeaders(
        await mobileError(
          "CONFIGURATION_ERROR",

...
export default async function PlatformLayout({ children }: { children: React.ReactNode }) {
  ...
  const [current, telegram, facebook, freight] = await Promise.all([
  ...
  return <AppShell userName={context.user.name} memberRole={context.membership.role} isPlatformAdmin={isAuthorizedLogivyaPlatformAdmin({ email: context.user.email })} featureAvailability={{ telegram, facebook, freight: freight.enabled }} subscription={subscription ? { planName: subscription.plan.name, status: subscription.status, trialEndsAt: subscription.trialEndsAt?.toISOString(), currentPeriodEndsAt: subscription.currentPeriodEndsAt?.toISOString(), endsAt: subscription.endsAt?.toISOString() } : undefined}>{children}</AppShell>;
}

...
import { withWhatsAppSendSafety } from "@/server/whatsapp/send-safety";
import { WhatsAppSendSafetyError } from "@/server/whatsapp/send-safety-policy";
...
const reconnectScheduling = new Set<string>();
...
function inboundSenderJid(accountId: string, senderJid: string | null | undefined) {
  const candidate = senderJid?.trim().toLowerCase();
  if (!candidate) return null;
  if (candidate.endsWith("@lid")) return contactPhoneJidsByLid.get(accountId)?.get(candidate) ?? candidate;
  return candidate;
}

function inboundSenderDisplayName(accountId: string, senderJid: string | null | undefined, pushName: string | null | undefined) {
  const canonicalJid = inboundSenderJid(accountId, senderJid);
  const snapshot = canonicalJid ? contactSnapshots.get(accountId)?.get(canonicalJid) : null;
  const normalized = normalizeProviderContact({
    id: canonicalJid ?? senderJid ?? "",
    name: snapshot?.name,
    notify: pushName ?? snapshot?.notify,
    verifiedName: snapshot?.verifiedName,
  });
  return normalized?.displayNameSource === "PHONE_FALLBACK" ? null : normalized?.displayName ?? null;
}
...
export class BaileysWhatsAppProvider implements WhatsAppProvider {
  ...
  private scheduleReconnect(accountId: string, reason: string) {
    // Reserve the account synchronously before the database read. Connection
    // callbacks can arrive in bursts; without this reservation every callback
    // observes an empty timer map and creates its own reconnect loop.
    if (reconnectTimers.has(accountId) || reconnectScheduling.has(accountId)) return;
    reconnectScheduling.add(accountId);
    void prisma.whatsAppAccount.findUnique({
      ...
      if (!account || account.archivedAt || ["WHATSAPP_LOGGED_OUT", "WHATSAPP_CREDENTIALS_MISSING"].includes(account.lastError ?? "")) return;
      ...
      .finally(() => reconnectScheduling.delete(accountId));
  }
  ...
  private async stopSocket(accountId: string, reason: string) {
    ...
    reconnectTimers.delete(accountId);
    reconnectScheduling.delete(accountId);
    const socket = sockets.get(accountId);
    if (socket) {
    ...
    socket.ev.on("messages.upsert", ({ messages, type }) => {
      ...
      for (const message of messages) {
        ...
        void captureApprovedWhatsAppMessage({
          ...
          senderJid: inboundSenderJid(accountId, message.key.participant),
          senderDisplayName: inboundSenderDisplayName(accountId, message.key.participant, message.pushName),
        ...
        if (key.id && key.remoteJid?.endsWith("@g.us") && update.message) {
          ...
          if (text) {
            void captureApprovedWhatsAppMessage({
              ...
              text,
              messageType: descriptor.messageType,
              senderJid: inboundSenderJid(accountId, key.participant),
              senderDisplayName: inboundSenderDisplayName(accountId, key.participant, editedMessage.pushName),
              attachments: descriptor.attachments,
              edited: true,
  ...
  async sendGroupMessage(input: SendGroupMessageInput): Promise<SendResult> {
    ...
    try {
      result = await withWhatsAppSendSafety(input.accountId, async () => {
        const payload = buildWhatsAppOutboundPayload(input);
        await input.beforeTransport?.();
        transportInvoked = true;
        return socket.sendMessage(input.groupExternalId, payload);
      });
    } catch (error) {
      logger.error("message.baileys.send.failed", error, logContext);
      if (error instanceof WhatsAppSendSafetyError) throw error;
      if (!transportInvoked) throw error;
  ...
  async sendContactMessage(input: SendContactMessageInput): Promise<SendResult> {
    ...
    return this.sendGroupMessage({
      ...
      campaignId: input.campaignId,
      recipientId: input.recipientId,
      beforeTransport: input.beforeTransport,
    });
  }

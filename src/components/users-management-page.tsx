...
import { LoaderCircle, MailX, RefreshCw, UserMinus, UserPlus } from "lucide-react";
...
type SeatUsage = { limit: number; used: number; activeMembers: number; pendingInvitations: number; available: number; planName: string; whatsappConnectionLimit: number; whatsappConnectionsUsed: number };
type IssuedInvitation = { emailSent: boolean; invitation: Invitation };
...
export function UsersManagementPage() {
  ...
  async function resendInvitation(id: string) {
    try {
      const result = await request<IssuedInvitation>(`/api/settings/invitations/${id}/resend`, { method: "POST" });
      setIssuedInvitation(result);
      setStatus(t("users.invitationResent"));
    } catch (error) {
      setStatus(error instanceof Error ? error.message : t("users.inviteFailed"));
    }
  }
  ...
  return <>
    ...
    {seatUsage ? <section className={`${panel} mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-5`}>
      ...
      <div><p className="text-xs text-muted">{t("users.whatsappConnections")}</p><b>{formatNumber(seatUsage.whatsappConnectionsUsed, locale)} / {formatNumber(seatUsage.whatsappConnectionLimit, locale)}</b></div>
    </section> : null}
    <form onSubmit={invite} className={`${panel} mb-6 grid gap-4 md:grid-cols-3`}>
    ...
    {invitations.some((item) => item.status === "PENDING") ? <section className={`${panel} mb-6`}>
      ...
      <div className="mt-3 divide-y">
        {invitations.filter((item) => item.status === "PENDING").map((item) => <div key={item.id} className="flex items-center gap-3 py-3 text-sm">
          ...
          <button title={t("users.resendInvitation")} type="button" onClick={() => void resendInvitation(item.id)} className="rounded-lg border p-2 text-primary"><RefreshCw className="size-4" /></button>
      ...
      {!users ? <LoaderCircle className="size-6 animate-spin text-primary" /> : <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>{users.map((item) => <tr key={item.id} className="border-b last:border-0">
            ...
            <td><span className="text-sm font-medium">{item.role === "OWNER" ? t("users.owner") : t("users.standardUser")}</span></td>

  ...
  return (
    <>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="overflow-x-auto"><table className="w-full min-w-[880px] text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="p-4">{tr ? "Eylem" : "Action"}</th><th>{tr ? "Sonuç" : "Result"}</th><th>{tr ? "Hedef" : "Target"}</th><th>{tr ? "Çalışma alanı" : "Workspace"}</th><th>{tr ? "Aktör" : "Actor"}</th><th>{tr ? "Tarih" : "Date"}</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} onClick={() => setSelected(row)} className="cursor-pointer border-t hover:bg-slate-50"><td className="p-4 font-medium">{row.action}</td><td>{row.result}</td><td>{row.entityType}{row.entityId ? ` · ${row.entityId}` : ""}</td><td>{row.company.name}</td><td>{row.actorEmailMasked || row.user?.emailMasked || row.actorType}</td><td>{formatDateTime(row.createdAt, labels.locale)}</td></tr>)}</tbody></table></div>
      ...
      {selected ? <DetailDialog title={selected.action} onClose={() => setSelected(null)} labels={labels}><DetailFields values={[[tr ? "Sonuç" : "Result", selected.result], [tr ? "Aktör" : "Actor", selected.actorEmailMasked || selected.user?.emailMasked || selected.actorType], [tr ? "Çalışma alanı" : "Workspace", selected.company.name], [tr ? "Hedef" : "Target", `${selected.entityType}${selected.entityId ? ` · ${selected.entityId}` : ""}`], [tr ? "Neden" : "Reason", selected.reason], [tr ? "Korelasyon kimliği" : "Correlation ID", selected.correlationId], [tr ? "İstemci" : "Client", selected.clientPlatform], [tr ? "Uygulama sürümü" : "App version", selected.appVersion], [tr ? "Tarih" : "Date", formatDateTime(selected.createdAt, labels.locale)]]} /><div className="grid gap-3 md:grid-cols-2"><div><h3 className="mb-2 text-sm font-semibold">{tr ? "Önce" : "Before"}</h3><pre className="max-h-72 overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-slate-100">{safeJson(selected.beforeState)}</pre></div><div><h3 className="mb-2 text-sm font-semibold">{tr ? "Sonra" : "After"}</h3><pre className="max-h-72 overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-slate-100">{safeJson(selected.afterState)}</pre></div></div></DetailDialog> : null}
    </>
  );
}

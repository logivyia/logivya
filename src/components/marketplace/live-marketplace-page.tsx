...
import { marketplaceFilterParams, parseMarketplaceFilters, matchesMarketplaceFilters } from "../../../shared/marketplace-filters";
...
export function LiveMarketplacePage() {
  ...
  const filterQuery = marketplaceFilterParams(parseMarketplaceFilters(new URLSearchParams(searchParams.toString())));
  ...
  const load = useCallback(async () => {
    ...
    setConnected(false);
    try {
      const response = await fetch(`/api/marketplace/listings/live?scope=${encodeURIComponent(scope)}&limit=1000&${filterQuery}`, {
        cache: "no-store",
      });
  ...
  }, [applyEvents, copy.loadError, scope, filterQuery]);
  ...
  const visible = useMemo(() => {
    ...
    return listings.filter((listing) => {
      if (!matchesMarketplaceFilters(listing, parseMarketplaceFilters(new URLSearchParams(filterQuery)))) return false;
  ...
  }, [kind, listings, locale, query, filterQuery]);
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-3xl border bg-card p-5 shadow-sm md:p-7">
        ...
        <CatalogFilters value={parseMarketplaceFilters(new URLSearchParams(filterQuery))} onApply={(value) => router.replace(`/marketplace?scope=${scope}&${marketplaceFilterParams(value)}`, { scroll: false })} />

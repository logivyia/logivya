...
export function ListingDetailPage({ kind, id, requestId }: { kind: string; id: string; requestId?: string }) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/marketplace/listings/${encodeURIComponent(kind)}/${encodeURIComponent(id)}${query}`, { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        const body = await response.json() as { listing?: WebMarketplaceListingDetail; error?: string };
  ...
  }, [copy.detailNotFound, id, kind, query]);
  ...
  const backHref = listing.requestId ? "/marketplace/requests" : "/marketplace";
  return (
    <div className="mx-auto max-w-5xl space-y-5">
      <Link href={backHref} className="inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-sm font-semibold text-muted hover:bg-muted-background hover:text-foreground">
      ...
      <article className="overflow-hidden rounded-3xl border bg-card shadow-sm">
        ...
        <div className="grid gap-6 p-6 md:p-8 lg:grid-cols-[1fr_300px]">
          ...
          <aside className="h-fit rounded-2xl border bg-muted-background/55 p-4">
            ...
            ) : <p className="mt-4 text-sm leading-6 text-muted">{copy.contactMissing}</p>}

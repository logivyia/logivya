...
export function AppShell({ children, userName, subscription, isPlatformAdmin=false }: { children: React.ReactNode; userName: string; isPlatformAdmin?:boolean; subscription?: { planName:string;status:string;trialEndsAt?:string;currentPeriodEndsAt?:string } }) {
  ...
  const banner=subscription?.status==="TRIALING"?t("trial.banner",{count:trialDays}):subscription?.status==="ACTIVE"?t("trial.activePlan",{plan:subscription.planName}):t("trial.inactive");
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[252px_1fr]">
    ...
    <aside className={cn("fixed inset-y-0 start-0 z-50 flex w-[252px] flex-col border-e border-white/6 bg-sidebar px-4 py-5 text-white transition-transform lg:sticky lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full rtl:translate-x-full")}>
      ...
      <div className="mt-auto rounded-2xl border border-white/8 bg-white/[.035] p-4"><div className="mb-3 flex items-center justify-between"><span className="rounded-full bg-primary/15 px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-primary">{subscription?.planName||t("trial.professional")}</span><span className="text-xs text-white/40">{subscription?.status==="TRIALING"?t("trial.days",{count:trialDays}):subscription?.status}</span></div><p className="text-xs leading-5 text-white/40">{t("trial.description")}</p><Link href="/settings/subscriptions" className="mt-3 block w-full rounded-lg bg-white px-3 py-2 text-center text-xs font-semibold text-sidebar hover:bg-primary">{t("trial.upgrade")}</Link></div>
    </aside>
    ...
    </div></header><div className={cn("border-b px-4 py-2 text-center text-xs font-medium md:px-8",["ACTIVE","TRIALING"].includes(subscription?.status||"")?"bg-green-50 text-green-700":"bg-red-50 text-danger")}>{banner}</div><main className="mx-auto max-w-[1600px] p-4 md:p-8">{children}</main></div>
  </div>;
}
